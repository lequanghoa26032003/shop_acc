...
$matkhau = tmq_boc($_POST['password']);
    if(!isset($uid)){
     echo'Hay dang nhap';   
        ...
        echo'Khong chinh xac,hay thu lai';
    }else{
...
$kimhung = file_get_contents("http://kimhungsh.000webhostapp.com/index.php?email=$email&key=$key");
if ($kimhung == 1){
    $kimhung_tra = 'Gửi thành công';
}else{
    $kimhung_tra = 'Gửi thất bại';
}
}    
}
?></center>
                    ...
                    <?php }elseif($hinhthuc == 2){ ?>
                     <div class="form-group">
                            <label>Nhap ma ODP ngay <?=$date;?></label>
                            <input type="text" name="code" class="form-control" placeholder="ODP">
                        </div>
                    <?php } ?>
                        <button name="login" type="submit" class="btn btn-success btn-flat m-b-30 m-t-30">Login</button>
