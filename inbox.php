<?php
$mysqli = new mysqli("sql203.byethost.com","b7_24773420","Haoihungne1","b7_24773420_a1");
$mysqli -> set_charset("utf8");
ob_start('ob_gzhandler');
date_default_timezone_set('Asia/Ho_Chi_Minh');


$cookie = 'c_user=100027893116616; xs=5%3ADR1gMI7RJ-ggXg%3A2%3A1594473404%3A9563%3A6177'; /// nhập cookie của bạn.
if(isset($_GET[save])) {
    $code =$_POST[msg];
       $data0 =$mysqli->query("SELECT * FROM bot WHERE `msg` = '".$ktra[msg]."'  ")->fetch_assoc();
if(!$data0) {
...
?><script>function save() {$.ajax({url : '/sms.php?save',type : 'POST',data : {msg : $('#msg').val(), text : $('#text').val() },success : function(result){$('#result').html(result);}});$('#text').val('');$('#msg').val('');}</script><?PHP
return false;
}
...
$ten = explode("</span>",explode('<span class="bl">',$getlink)[1])[0];
...
if($noidung == "hotro" or $noidung =="HOTRO" or $noidung =="Hotro" or $noidung == "HoTro") {
        $body = 'Xin chào anh/chị. Mình là BOT Chát được cài đặt và chạy trên nền tảng PHP 7.4. Rất vui được chát giao lưu với bạn. Bạn có bất kì câu hỏi gì, hoặc muốn trò chuyện nhắn tin với mình thì cứ nhắn lại xuống dưới nhé.
          Yêu bạn.';
    } else {
    $bot = $mysqli->query("SELECT * FROM `bot` WHERE `msg` like '%$noidung%'")->fetch_assoc();
    if($bot[id] <=0) {
        $body = 'BOT Nghĩa : Huhu, Nghĩa chưa dạy tớ câu hỏi này. Để biết thêm chi tiết cậu hãy chát HOTRO để xem thêm nha.
        Bây giờ là : '.date('h:m:s d/m/Y').'';
    } else {
        $body = $bot[text];
    }
    }
