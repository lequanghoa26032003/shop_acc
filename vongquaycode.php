...
<h3 class="c-center c-font-uppercase c-font-bold">Vòng Quay Code</h3>
...
<img style="width: 80%;max-width: 80%;opacity: 1;" src="https://demo01.thuecodeweb.net/storage/images/n4AypVI.png" alt="Play" id="rotate-play">
</div>
</section>
</div>
</div>
...
<a href="/luckywheel/logacc9" class="btn btn-success m-btn m-btn--custom m-btn--icon m-btn--pill">
<span>
<i class="la la-cloud-upload"></i>
...
<?php
$get = $db->query("SELECT * FROM `CVH_quaycode` WHERE `loai` != '0' ORDER BY id DESC LIMIT 0,20");
if($get != null){
foreach($get as $ls){
?>			<tr>
<td><?= str_replace( substr(($ls['nguoiquay']), -3), '***', ($ls['nguoiquay']) );?></td>
<td>Trúng <?=number_format($ls['loai']);?><sup>!</sup></td>
<td><?=$ls['date'];?></td></tr>
<?php }
 }else{?>
     <tr>
