...
if(isset($_POST['timkiem'])){
$get = $db->query("SELECT * FROM `TMQ_biendoi` WHERE `uid` = '$uid' ORDER BY id DESC $start $end LIMIT $from,$sotin1trang");
}else{
$get = $db->query("SELECT * FROM `TMQ_biendoi` WHERE `uid` = '$uid' ORDER BY id DESC LIMIT $from,$sotin1trang");
}
foreach($get as $bd){
?>				<tr>
<td><?=$bd['id'];?></td>
