...
<div class="c-layout-sidebar-content ">
				...
				<div class="c-content-title-1">
					<h3 class="c-font-uppercase c-font-bold">Lịch sử vòng quay vũ khí ước mơ</h3>
...
if(isset($_POST['timkiem'])){
$get = $db->query("SELECT * FROM `CVH_quayvk` WHERE `nguoiquay` = '$uid' ORDER BY id DESC $start $end LIMIT $from,$sotin1trang");
}else{
$get = $db->query("SELECT * FROM `CVH_quayvk` WHERE `nguoiquay` = '$uid' ORDER BY id DESC LIMIT $from,$sotin1trang");
}
...
<?php } ?>
		...
		<?php 
$tong = $db->query("SELECT * FROM `CVH_quayvk` WHERE `nguoiquay` = '$uid' ")->rowcount();

if ($tong > $sotin1trang){
echo '<center>' . phantrang('/rubywheel/logacc/2907?', $from, $tong, $sotin1trang) . '</center>';
} ?>	</div></div></div></div>
