...
function xoakytu($chuoi)
{
    $ketqua = preg_replace('/([^\pL\.\ ]+)/u', '', strip_tags($chuoi));
    return $ketqua;
}
//xóa file
...
//status card
function status_luauytin($status)
{
    $str = array();
    switch ($status) {
        case 99:
            $str['status']  = "Chờ xử lý";
            $str['color']   = "#337ab7";
            break;
        case 0:
            $str['status']  = "Chờ xử lý";
            ...
            break;
        case 2:
            $str['status']  = "Thành công";
            $str['color']   = "#599414";
            break;
        case 3:
            $str['status']  = "Sai m.giá";
            $str['color']   = "#A73939";
            break;
        case 1:
            $str['status']  = "Thất bại";
...
//lấy dữ liệu cài đặt
function caidat_lattheff($text2)
{
    global $db;
    $cd2 = $db->query("SELECT * FROM `HK_lattheff` WHERE $text2")->fetch();
    return html_entity_decode($cd2['id']);
}
//sendmail
function sendmail($email, $chu_de, $body)
{
    $fromserver = 'kimhungtruongnguyen@gmail.com';
    $pass = 'phamhuongphuongthi';
    $port = 587;
